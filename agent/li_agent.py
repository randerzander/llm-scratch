...
from llm_scratch import qwen
from llm_scratch import llm as scratch_llm
...
class OurCustomLLM(CustomLLM):
    ...
    def complete(self, prompt: str, **kwargs: Any) -> CompletionResponse:
        print(prompt)
        return CompletionResponse(text=scratch_llm(prompt))
    ...
    def stream_complete(self, prompt: str, **kwargs: Any) -> Iterator[CompletionResponse]:
        ...
        print(prompt)
        yield CompletionResponse(text=scratch_llm(prompt))
...
class DuckDuckGoRetriever(BaseRetriever):
    def _retrieve(self, query_bundle):
        ...
        results = find_relevant_content(query, content, chunk_size=200, k=5)
        nodes = []
...
if __name__ == "__main__":
    print("Loading LLM...")
    qwen()
    print("LLM loaded, starting agent...")
    asyncio.run(q("How many hotdogs would it take to reach the moon?"))
